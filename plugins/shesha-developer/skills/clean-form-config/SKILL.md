...
- **Step 4k** — Apply `scriptRules` from `base.json` against all script strings: detect `window.location` (error), `globalState` (error), IIFEs (warning), and the Promise constructor anti-pattern (warning); skip `no-console-log` and `no-then-chain` (handled by 4b/4j).
- **Step 5 / 5b / 5c / 5d / 5e / 5f / 5g / 5h / 5i / 5j / 5k** — Present findings (dead props, console.log, type mismatches, values shape issues, layout issues, label references, script syntax errors, missing try-catch, missing async/promise, .then() chaining).
